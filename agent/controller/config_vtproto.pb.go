...
func (this *Config) EqualVT(that *Config) bool {
	...
	if this.NodePeerId != that.NodePeerId {
		return false
	}
...
func (m *Config) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.NodePeerId) > 0 {
		i -= len(m.NodePeerId)
		copy(dAtA[i:], m.NodePeerId)
		i = encodeVarint(dAtA, i, uint64(len(m.NodePeerId)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.PrivKey) > 0 {
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field NodePeerId", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.NodePeerId = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
