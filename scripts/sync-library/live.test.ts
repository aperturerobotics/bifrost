...
const schema = defineSchema({
  id: 'live-acceptance',
  version: 1,
  collections: { todos: text },
  mutations: {},
})

test(
  'packed clients observe browser-style and server writes and close naturally',
  { timeout: 30_000 },
  async () => {
    ...
    const server = await createServer({
      ...
      schema,
      mutations: {},
      authenticate: async (token) => ({ subject: token, scope: 'team' }),
      authorize: () => true,
    })
    ...
    try {
      ...
      const stop = clients[1].collection('todos').subscribe(
        {},
        {
          next: (state) => {
            ...
            if (
              ...
              state.data.some((entry) => entry.key === 'server')
            )
              expected.resolve()
          },
      ...
      assert.equal(
        await clients[1].collection('todos').get('browser'),
        'Ship the RC',
      )
      await server
