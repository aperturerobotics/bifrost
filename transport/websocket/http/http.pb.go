...
func (m *Config) CloneVT() *Config {
	...
	r.Quic = m.Quic.CloneVT()
	r.Verbose = m.Verbose
	if rhs := m.HttpPatterns; rhs != nil {
		r.HttpPatterns = slices.Clone(rhs)
	}
	if rhs := m.PeerHttpPatterns; rhs != nil {
		r.PeerHttpPatterns = slices.Clone(rhs)
	}
	if rhs := m.Dialers; rhs != nil {
		r.Dialers = make(map[string]*dialer.DialerOpts, len(rhs))
		for k, v := range rhs {
			r.Dialers[k] = v.CloneVT()
		}
	}
...
func (this *Config) EqualVT(that *Config) bool {
	...
	if len(this.HttpPatterns) != len(that.HttpPatterns) {
		return false
	}
	for i, vx := range this.HttpPatterns {
		vy := that.HttpPatterns[i]
		if vx != vy {
			return false
		}
	}
	if len(this.PeerHttpPatterns) != len(that.PeerHttpPatterns) {
		return false
	}
	for i, vx := range this.PeerHttpPatterns {
		vy := that.PeerHttpPatterns[i]
		if vx != vy {
			return false
		}
	}
...
func (m *Config) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.PeerHttpPatterns) > 0 {
		for iNdEx := len(m.PeerHttpPatterns) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.PeerHttpPatterns[iNdEx])
			copy(dAtA[i:], m.PeerHttpPatterns[iNdEx])
			i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(len(m.PeerHttpPatterns[iNdEx])))
			i--
			dAtA[i] = 0x1a
		}
	}
	if len(m.HttpPatterns) > 0 {
		for iNdEx := len(m.HttpPatterns) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.HttpPatterns[iNdEx])
			copy(dAtA[i:], m.HttpPatterns[iNdEx])
			i = protobuf_go_lite.EncodeVarint(dAtA, i, uint64(len(m.HttpPatterns[iNdEx])))
			i--
			dAtA[i] = 0x12
		}
	}
...
func (m *Config) SizeVT() (n int) {
	...
	if len(m.HttpPatterns) > 0 {
		for _, s := range m.HttpPatterns {
			l = len(s)
			n += 1 + l + protobuf_go_lite.SizeOfVarint(uint64(l))
		}
	}
	if len(m.PeerHttpPatterns) > 0 {
		for _, s := range m.PeerHttpPatterns {
			l = len(s)
			n += 1 + l + protobuf_go_lite.SizeOfVarint(uint64(l))
		}
	}
...
func (x *Config) MarshalProtoText() string {
	...
	if len(x.PeerHttpPatterns) > 0 {
		...
		sb.WriteString("peer_http_patterns: [")
	...
	if len(x.Dialers) > 0 {
		...
		for _, k := range slices.Sorted(maps.Keys(x.Dialers)) {
			v := x.Dialers[k]
			sb.WriteString(" ")
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 2:
			...
			m.HttpPatterns = append(m.HttpPatterns, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		case 3:
			if wireType != 2 {
			...
			m.PeerHttpPatterns = append(m.PeerHttpPatterns, string(dAtA[iNdEx:postIndex]))
			iNdEx = postIndex
		case 4:
			if wireType != 2 {
