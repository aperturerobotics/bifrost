...
func (m *DialerOpts) CloneMessageVT() any {
	return m.CloneVT()
}
...
func (m *DialerOpts) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: DialerOpts: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: DialerOpts: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field Address", wireType)
			}
