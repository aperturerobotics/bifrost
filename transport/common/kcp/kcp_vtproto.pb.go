...
func (m *Opts) CloneVT() *Opts {
	...
	r := new(Opts)
	r.DataShards = m.DataShards
	r.ParityShards = m.ParityShards
	r.Mtu = m.Mtu
	r.KcpMode = m.KcpMode
	r.BlockCrypt = m.BlockCrypt
	r.BlockCompress = m.BlockCompress
	r.StreamMuxer = m.StreamMuxer
...
func (m *Opts) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.StreamMuxer != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.StreamMuxer))
		i--
		dAtA[i] = 0x38
	}
	if m.BlockCompress != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.BlockCompress))
		i--
		dAtA[i] = 0x30
	}
	if m.BlockCrypt != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.BlockCrypt))
		i--
		dAtA[i] = 0x28
	}
	if m.KcpMode != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.KcpMode))
		i--
		dAtA[i] = 0x20
	}
	if m.Mtu != 0 {
	...
	if m.ParityShards != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.ParityShards))
		i--
		dAtA[i] = 0x10
	}
	if m.DataShards != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.DataShards))
		i--
		dAtA[i] = 0x8
	}
...
func (m *Opts) SizeVT() (n int) {
	...
	_ = l
	if m.DataShards != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.DataShards))
	}
	if m.ParityShards != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.ParityShards))
	}
	if m.Mtu != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.Mtu))
	}
	if m.KcpMode != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.KcpMode))
	}
	if m.BlockCrypt != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.BlockCrypt))
	}
	if m.BlockCompress != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.BlockCompress))
	}
	if m.StreamMuxer != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.StreamMuxer))
	}
