...
func (m *HandshakeExtraData) CloneVT() *HandshakeExtraData {
	...
	r := new(HandshakeExtraData)
	r.LocalTransportUuid = m.LocalTransportUuid
...
func (this *HandshakeExtraData) EqualVT(that *HandshakeExtraData) bool {
	...
	if this.LocalTransportUuid != that.LocalTransportUuid {
		return false
	}
...
func (this *HandshakeExtraData) EqualMessageVT(thatMsg proto.Message) bool {
	that, ok := thatMsg.(*HandshakeExtraData)
	if !ok {
		return false
	}
...
func (m *HandshakeExtraData) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.LocalTransportUuid != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.LocalTransportUuid))
		i--
		dAtA[i] = 0x8
	}
...
func (m *HandshakeExtraData) SizeVT() (n int) {
	...
	_ = l
	if m.LocalTransportUuid != 0 {
		n += 1 + protohelpers.SizeOfVarint(uint64(m.LocalTransportUuid))
	}
...
func (m *HandshakeExtraData) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return fmt.Errorf("proto: HandshakeExtraData: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return fmt.Errorf("proto: HandshakeExtraData: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field LocalTransportUuid", wireType)
			}
			m.LocalTransportUuid = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return protohelpers.ErrIntOverflow
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.LocalTransportUuid |= uint64(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
