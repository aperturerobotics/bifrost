...
function createBaseConfig(): Config {
  return { transportPeerId: "", packetOpts: undefined, dialers: {} };
}
...
export const Config = {
  encode(message: Config, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    ...
    if (message.packetOpts !== undefined) {
      Opts.encode(message.packetOpts, writer.uint32(18).fork()).ldelim();
    }
    Object.entries(message.dialers).forEach(([key, value]) => {
      Config_DialersEntry.encode({ key: key as any, value }, writer.uint32(26).fork()).ldelim();
    });
    return writer;
  },
