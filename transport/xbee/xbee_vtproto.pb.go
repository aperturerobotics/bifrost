...
func (this *Config) EqualVT(that *Config) bool {
	...
	if this.DevicePath != that.DevicePath {
		return false
	}
	if this.DeviceBaud != that.DeviceBaud {
		return false
	}
	if equal, ok := interface{}(this.PacketOpts).(interface{ EqualVT(*kcp.Opts) bool }); ok {
	...
	for i := range this.Dialers {
		if equal, ok := interface{}(this.Dialers[i]).(interface{ EqualVT(*dialer.DialerOpts) bool }); ok {
			if !equal.EqualVT(that.Dialers[i]) {
				return false
			}
		} else if !proto.Equal(this.Dialers[i], that.Dialers[i]) {
			return false
		}
	}
...
func (m *Config) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.DeviceBaud != 0 {
		i = encodeVarint(dAtA, i, uint64(m.DeviceBaud))
		i--
		dAtA[i] = 0x18
	}
	if len(m.DevicePath) > 0 {
		...
		i = encodeVarint(dAtA, i, uint64(len(m.DevicePath)))
		i--
		dAtA[i] = 0x12
	}
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field DeviceBaud", wireType)
			}
			m.DeviceBaud = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.DeviceBaud |= int32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
