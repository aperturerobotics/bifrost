...
func (this *Packet) EqualVT(that *Packet) bool {
	...
	for i := range this.Subscriptions {
		if !this.Subscriptions[i].EqualVT(that.Subscriptions[i]) {
			return false
		}
	}
	...
	for i := range this.Publish {
		if equal, ok := interface{}(this.Publish[i]).(interface{ EqualVT(*peer.SignedMsg) bool }); ok {
			if !equal.EqualVT(that.Publish[i]) {
				return false
			}
		} else if !proto.Equal(this.Publish[i], that.Publish[i]) {
			return false
		}
	}
	return string(this.unknownFields) == string(that.unknownFields)
