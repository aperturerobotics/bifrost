...
func (m *PubMessageInner) CloneMessageVT() any {
	return m.CloneVT()
}
...
func (m *PubMessageInner) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		if wireType == 4 {
			return errors.Errorf("proto: PubMessageInner: wiretype end group for non-group")
		}
		if fieldNum <= 0 {
			return errors.Errorf("proto: PubMessageInner: illegal tag %d (wire type %d)", fieldNum, wire)
		}
		switch fieldNum {
		case 1:
			if wireType != 2 {
		...
		case 2:
			if wireType != 2 {
				return errors.Errorf("proto: wrong wireType = %d for field Channel", wireType)
			}
