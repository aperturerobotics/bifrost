...
func (m *Packet_Ciphertext) CloneVT() *Packet_Ciphertext {
	...
	r := new(Packet_Ciphertext)
	r.ReceiverKeyKnown = m.ReceiverKeyKnown
...
func (m *Packet) CloneVT() *Packet {
	...
	r.InitPkt = m.InitPkt.CloneVT()
	r.InitAckPkt = m.InitAckPkt.CloneVT()
	r.CompletePkt = m.CompletePkt.CloneVT()
	if len(m.unknownFields) > 0 {
		r.unknownFields = make([]byte, len(m.unknownFields))
		copy(r.unknownFields, m.unknownFields)
	}
...
func (m *Packet_Init) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SenderEphPub) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.SenderEphPub)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.ReceiverPeerId) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ReceiverPeerId)))
		i--
		dAtA[i] = 0x12
	}
...
func (m *Packet_Ciphertext) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if len(m.SenderPubKey) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.SenderPubKey)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.TupleSignature) > 0 {
		...
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.TupleSignature)))
		i--
		dAtA[i] = 0xa
	}
	return len(dAtA) - i, nil
