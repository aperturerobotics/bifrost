...
func (m *Config) CloneVT() *Config {
	...
	if rhs := m.ServiceIdPrefixes; rhs != nil {
		r.ServiceIdPrefixes = slices.Clone(rhs)
	}
...
func (x *Config) MarshalProtoText() string {
	...
	if len(x.ServiceIdPrefixes) > 0 {
		if sb.Len() > 8 {
			sb.WriteString(" ")
		}
		sb.WriteString("service_id_prefixes: [")
		for i, v := range x.ServiceIdPrefixes {
			if i > 0 {
				sb.WriteString(", ")
			}
...
func (m *Config) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 3:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field ServiceIdPrefixes", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
